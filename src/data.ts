import { ColorPalette, Option, SliderConfig } from './types.js';

export const photoTypes: Option[] = [
...
export const styleCategories: Option[] = [
...
export const styles: Option[] = Array.from({ length: 200 }, (_, index) => {
...
const makeOptions = (prefix: string, categories: string[], count: number, samples: string[]): Option[] =>
...
export const accessoryModes: Option[] = ['不加配件', '保留原配件', '替換配件', '新增 1 個配件', '新增 2–3 個配件', '主題配件組合'].map((label, index) => ({ id: `accessory-mode-${index}`, label, prompt: label }));

export const colorPalettes: ColorPalette[] = makeOptions('palette', ['奶油色', '黑金', '粉色', '藍灰', '莫蘭迪', '復古棕', '霓虹色', '森林綠', '海鹽藍', '玫瑰金'], 100, ['柔奶油', '黑金精品', '淡粉甜感', '藍灰專業', '莫蘭迪柔霧', '復古棕調', '霓虹撞色', '森林自然', '海鹽清新', '玫瑰金']).map((option, index) => ({
  ...option,
...
export const textStyles: Option[] = ['極簡現代', '高級精品', '韓系清新', '日系文青', '商務正式', '潮流個性', '可愛活潑', '復古海報', '科技未來', '新中式'].map((label, index) => ({ id: `text-style-${index}`, label, prompt: `${label}字體風格，文字清楚可讀且與圖片融合` }));
...
export const expertSliders: SliderConfig[] = [
  ...basicSliders,
...
export const frameSliders: SliderConfig[] = [
...
export const textSliders: SliderConfig[] = [
