...
import { GeneratorState, PromptResult, SliderKey } from './types.js';

const findLabel = <T extends { id: string; label: string; prompt: string }>(items: T[], id: string): T => items.find((item) => item.id === id) ?? items[0];
const level = (value: number) => (value >= 75 ? '高' : value >= 45 ? '中' : '低');

const sliderNames: Partial<Record<SliderKey, string>> = {
...
export const createInitialState = (mode: GeneratorState['mode']): GeneratorState => ({
  mode,
  photoType: 'single',
  age: 'age-0',
...
export const assemblePrompt = (state: GeneratorState): PromptResult => {
  ...
  const sliderSummary = Object.entries(state.sliders)
    ...
    .map(([key, value]) => `${sliderNames[key as SliderKey]}${level(value)}`)
    .join('、');
