...
async function copyText(text, message = '已複製') {
    ...
    const msg = root.querySelector('#copyMsg');
    if (msg)
        msg.textContent = `｜${message}`;
}
function comicCopyBox(title, desc, text, id) {
...
function renderComicSystem() {
    ...
    bindCommon();
    const copyMap = { system: comicSystemPrompt, episode: comicEpisodeTemplate, oneLine: comicOneLinePrompt };
    root.querySelectorAll('[data-copy-comic]').forEach((el) => el.onclick = () => copyText(copyMap[el.dataset.copyComic], '已複製，現在貼到 ChatGPT'));
}
